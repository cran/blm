...
The variance-covariance is estimated from an influence-based method \cite{Deville:1999}. The coefficients and variance-covariance can be extracted directly using \texttt{coef} and \texttt{vcov}.
...
As an additional way to evalute potential influential points in a BLM or LEXPIT model fit, each class has a method \texttt{dBeta}, which returns the jackknife deviates by subject and parameter. Here, we plot a sparkline-like figure of the packyear deviates by subject (Figure~\ref{fig1}).

<<eval=FALSE>>=
plot(y = dBeta(fit)[,2], x = 1:nrow(ccdata),
       type = "h")
@  

\begin{figure} \centering
\includegraphics[width=3.25in]{influence.pdf}
\label{fig1}
\caption{Jackknife deviates for variable \texttt{packyear} by subject}
\end{figure}
...
after baseline adjustment for the effects of $x_i'\beta$, what we can think of as `residual odds ratios'.
