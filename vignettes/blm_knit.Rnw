...
\texttt{data} arguments. For population-based estimates, we need to additionally include the design information on sample stratification and the sampling fractions. The following code fits a population-based linear risk model with additive effects due to female gender and packyears.
...
As an additional way to evalute potential influential points in a BLM or LEXPIT model fit, each class has a method \texttt{displacement}, which returns the confidence interval displacements by subject and parameter. Here, we plot a sparkline-like figure of the displacement measures of influence by subject (Figure~\ref{fig:1}).

<<fig=influence,eval=FALSE,fig=TRUE>>=
plot(y = displacement(fit), x = 1:nrow(ccdata),
       type = "h")
@  

\begin{figure} \centering
\includegraphics[height=.5\textwidth]{influence.pdf}
\caption{Confidence interval displacement by subject}
\label{fig:1}
\end{figure}
...
This suggest a lack of fit at the population level. When a model has more than 10 covariate classes, the Hosemer-Lemeshow test should be used. For the \texttt{blm} function, this test is implemented by the function \texttt{gof}.

\subsection{Mode of exposure's effect}

Because of the lack of fit of the additive model, we might suspect the linear assumption for pack-years. We could assess the functional relationship between risk and pack-years after accounting for the effect of gender by looking at excess risk. As a graphical diagnostic, we can compute the weighted average excess risk for the smaller model in bins defined by a categorical representaton of the explanatory variable. The function \texttt{excess.risk} performs this calculaton for \texttt{blm} or \texttt{lexpit} model objects. 

As an example, we use \texttt{excess.risk} to compute the binned residuals within each pack-year group with a BLM fit that has gender as the only explanatory variable. We then plot the excess risk against pack-years to see the shape of their functional relationship. The function returns the vector of the binned residuals with names equal to the categories of the grouping variable.

<<>>=
fit <- blm(y~female, data = ccdata,
             weight = ccdata$w, 
             strata = ccdata$strata)
...
<<label=excess,eval=FALSE,fig=TRUE>>=
...
\begin{figure} \centering
\includegraphics[height=.4\textwidth]{excess.pdf}
\caption{Excess risk plot against pack-years after accounting for gender in a BLM regressionanalysis.}
\label{fig:2}
\end{figure}

Figure~\ref{fig:2} suggests a non-linear relationship between risk adjusted for gender and pack-years smoked.

\section{Linear-Expit (LEXPIT) model}

\subsection{Model}

Suppose we were concerned that the linear assumption for pack-years might not be valid. If we thought that linearity on the relative risk scale was a more plausible model for the effect of pack-years, we could consider a LEXPIT model. The LEXPIT model describes the probability of $Y_i = 1$ as a function
